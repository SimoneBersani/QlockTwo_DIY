...
AmbientLightModule::AmbientLightModule(int _pinNumber, int _maxLightValue, int _minLightValue) : pinNumber(_pinNumber), maxLightValue(_maxLightValue), minLightValue(_minLightValue)
{
  for (int i = 0; i < 10; i++)
  {
...
int AmbientLightModule::getBrightness()
{
  ...
  Serial.println(lightSensorRaw);

  Serial.print("Light Sensor Filtered: ");
  Serial.println(lightSensorFiltered);

  if (lightSensorFiltered < minLightValue)
  {
    Serial.println("--> TOO DARK, TURNING OFF LIGHTS <--");
    return 2;
  }
